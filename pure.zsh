...
prompt_pure_async_tasks_ext() {
	...
	if zstyle -t ":prompt:pure:aws" show && [[ $oldaws_profile != $AWS_PROFILE || $oldaws_vault != $AWS_VAULT ]]; then
		typeset -g oldaws_profile=$AWS_PROFILE
		typeset -g oldaws_vault=$AWS_VAULT
		(( $+AWS_PROFILE )) && async_worker_eval "prompt_pure_ext" export AWS_PROFILE=$AWS_PROFILE
		(( $+AWS_VAULT   )) && async_worker_eval "prompt_pure_ext" export AWS_VAULT=$AWS_VAULT
		async_job "prompt_pure_ext" prompt_pure_async_aws_account
	fi

	if zstyle -t ":prompt:pure:k8s" show && [[ $oldkubeconfig != $KUBECONFIG ]]; then
		typeset -g oldkubeconfig=$KUBECONFIG
		async_worker_eval "prompt_pure_ext" export KUBECONFIG=$KUBECONFIG
